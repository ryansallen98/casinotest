...
const app = express();
const uri = 'https://relay.divvyden.com/v1?';
...
passport.use(
  new LocalStrategy(async (username, password, done) => {
    ...
    if (await bcrypt.compare(password, userPassword)) {
      return done(null, { username: username, key: newKey, isAdmin: isAdmin});
    } else {
  ...
  */
  ...
  fs.readFile(templatePath, 'utf8', (err, data) => {
    ...
    const invoiceData = {
      ...
      merchant: "Divvyden Concierge",
      recipient: "Divvyden Concierge",
      name: "Divvyden Concierge",
...
app.post("/admin/adduser", ensureAuthenticated, isAdmin, async (req, res) => {
...
app.post("/admin/removeuser", ensureAuthenticated, isAdmin, async (req, res) => {
  const { username } = req.body;
  console.log(username)
  usersDB.remove({ username }, {}, (err, numRemoved) => {
    if (err) {
      console.error(err);
...
app.get("/admin/users", ensureAuthenticated, isAdmin, async (req, res) => {
  ...
  usersDB.find({}, (err, users) => {
    if (err) {
      ...
      // filter out objects with isAdmin set to true
